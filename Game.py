...
node1 = node.node(213,64,47,128,1,screen)
node2 = node.node(213,192,47,128,2,screen)
node3 = node.node(213,320,47,128,3,screen)
...
for i in range(0, MapList.MapList[0].get_note_count()):
    if MapList.Click.get_long_note_length():
        NoteList.append(maps.note(MapList.Click.get_long_note_length(), 128, MapList.MapList[0].get_note(), speed=5, isLongNote=True))
        MapList.MapList[0].move_index()
        NoteList_Drawer.append(pygame.sprite.RenderPlain(NoteList[i]))

    else:
        NoteList.append(maps.note(27, 128, MapList.MapList[0].get_note(), speed=5))
        MapList.MapList[0].move_index()
        NoteList_Drawer.append(pygame.sprite.RenderPlain(NoteList[i]))
...
while not done:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_a:
                for i in range(maps.killed_note(0), Note_Count):
    ...
    screen.blit(number_font.render(str(maps.combo(0)), True, (0,0,0)), Setting_Value.Display_Set.combo)
